export function ThemeToggle({ isDark, onToggle }) {
  return (
    <button
      ...
      onClick={onToggle}
      className="group relative inline-flex h-12 w-[88px] items-center overflow-hidden rounded-full border border-black/10 bg-white/75 px-1.5 shadow-[0_10px_30px_rgba(15,15,20,0.08)] backdrop-blur-xl transition dark:border-white/10 dark:bg-white/5"
    >
      <span
      ...
      <span className="relative z-10 flex w-full items-center justify-between px-[9px] text-[13px] text-zinc-500 dark:text-white/45">
        <i className={`bi bi-brightness-high-fill transition ${isDark ? "opacity-0" : "opacity-45"}`} />
        <i className={`bi bi-moon-stars-fill transition ${isDark ? "opacity-45" : "opacity-0"}`} />
      </span>
    </button>
  );
}
